...
export interface IStorage {
  ...
  getConversations(userId: string): Promise<(Conversation & { 
    ...
    lastMessage?: Message & { user: User };
  })[]>;
  getConversation(conversationId: string): Promise<(Conversation & { 
    participants: (ConversationParticipant & { user: User })[];
    listing?: RoommateListing;
  }) | undefined>;
  getOrCreateListingConversation(listingId: string, userId1: string, userId2: string): Promise<Conversation>;
  getOrCreateHouseholdConversation(householdId: string): Promise<Conversation>;
  createConversationMessage(conversationId: string, userId: string, content: string): Promise<Message & { user: User }>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getConversations(userId: string): Promise<(Conversation & { 
    ...
    const conversationsWithDetails = await Promise.all(
      conversationsData.map(async (conv) => {
        ...
        // Get listing info if it's a listing conversation
        let listing: RoommateListing | undefined;
        if (conv.listingId) {
          const [listingData] = await db
            .select()
            .from(roommateListings)
            .where(eq(roommateListings.id, conv.listingId));
          listing = listingData;
        }
  ...
  async getOrCreateListingConversation(listingId: string, userId1: string, userId2: string): Promise<Conversation> {
    ...
    const existingConv = await db
      ...
      .having(sql`COUNT(DISTINCT ${conversationParticipants.userId}) = 2 
        AND SUM(CASE WHEN ${conversationParticipants.userId} IN (${userId1}, ${userId2}) THEN 1 ELSE 0 END) = 2`);

    if (existingConv.length > 0) {
      return existingConv[0].conversation;
    }
  ...
  async getOrCreateHouseholdConversation(householdId: string): Promise<Conversation> {
    // Check if household conversation already exists
  ...
  async createConversationMessage(conversationId: string, userId: string, content: string): Promise<Message & { user: User }> {
